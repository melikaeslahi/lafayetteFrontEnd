...
const ShowComment = ({ params }) => {
    const { data: comment = [] } =  useGetCommentPostQuery({ id:params.id});
    ...
    const [addNewAnswer, { data , isLoading: isSend ,isSuccess:Success }] =  useAddNewAnswerPostMutation()
    useEffect(() => {
        ...
        <>
            ...
            <InputContainer  
                ...
                <section className='flex flex-col basis-1/2 shadow-md shadow-pallete p-5 m-5 '>
                    ...
                    <h3> نام پست :  {comment.data?.commentable.title} کد پست:{comment.data?.commentable.id}    </h3>
                    <p>  متن کامنت:{comment.data?.body}  </p>
