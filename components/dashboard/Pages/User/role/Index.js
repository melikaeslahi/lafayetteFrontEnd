...
const Index = () => {
    ...
    const { data: roles = [], isError, isLoading, isSuccess } =  useGetAllRoleQuery({ page, perPage, search });
    ...
    useEffect(() => {
        ...
        dispatch(setItemLength( roles.data?.length));

    }, [isLoading, isSuccess, isError, roles])
    ...
    return (<>
        <TitlePage
            name='   نقش ها ها'
            sitemapPage='بخش  کاربران/ سطوح دسترسی/   نقش ها'
