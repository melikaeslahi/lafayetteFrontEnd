        ...
        <>
            ...
            <InputContainer>
                ...
                <section className='flex flex-col items-center justify-center  w-96 shadow-md shadow-pallete p-5 m-5 '>
                    ...
                    <h1 className='text-pallete text-lg'>     پرداخت کننده: {payment.data?.user_id ? payment.data?.user_id?.first_name + ' ' + payment.data?.user_id?.last_name : '-'}   </h1>
                    <h3>    مبلغ: {payment.data?.payments  ?payment.data?.payments.amount  : '-' }    </h3>
                    <p>  بانک: {payment.data?.payments  ?payment.data?.payments.gateway  : '-' }    </p>
                    <p>   شماره پرداخت  :{payment.data?.payments ?payment.data?.payments.transaction_id :'-'}  </p>
                    <p>   تاریخ پرداخت  :{payment.data?.payments ?payment.data?.payments.pay_date : '-'}  </p>
                    <p>    دریافت کننده پرداخت  :{payment.data?.payments ?payment.data?.payments.pay_date : '-'}  </p>
