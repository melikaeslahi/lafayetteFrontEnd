...
const CartItemFromDatabase = () => {
    const dispatch = useDispatch();
    const cart = useSelector(state => state.cart);
    const { user } = useSelector(state => state.auth);
    const [counter, setCounter] = useState(1);
    const [ counterChangeId, setCounterChangeId] = useState();

    const { data: cartItems, isSuccess } = useGetCartQuery();
    const [deleteCartItem, { data: deleteData }] = useDeleteCartMutation();


    useEffect(() => {
        if (deleteData?.status === 200) {
            toast('محصول با موفقیت از سبد خرید شما حذف شد', { position: 'bottom-left' })
        }
    }, [deleteData])
    ...
    return (
        <>
            <section className="flex flex-col m-2 p-2 ">
                {cartItems?.cartItems === null ? 'سبد خرید شما خالی می باشد' : cartItems?.cartItems.map((cart, index) => {
                        ...
                        <section key={index} className="grid grid-flow-row grid-cols-1 lg:grid-cols-4 xl:grid-cols-4 md:grid-cols-1 w-full h-full p-2 mt-2 border border-gray-300 rounded-lg">
                                ...
                                <section className="flex w-4/6 flex-col items-start">
                                    ...
                                    <section className="flex justify-between items-center mr-2 w-full">
                                        <section className="flex border rounded-lg  border-gray-300">
                                            { cart.id == counterChangeId  && cart.number == 1 ? <button
                                                  onClick={async() =>await deleteCartItem(cart.id)}
                                                className="p-1 rounded-lg m-1   text-lg text-pallete"
                                                type="button">
                                                ...
                                                onClick={()=> {
                                                    ...
                                                    setCounterChangeId(cart.id)}}
                                                className="p-1 rounded-lg m-1 bg-pallete text-lg text-white"
                                                type="button">-

                                            </button>}
                                            
                                            <input
                                             className="rounded-sm border-none w-8 p-0 text-center"
                                             type="number" 
                                             onChange={(e)=> cart.id == counterChangeId ? setCounter(e.target.value) : null}
                                              min="1" 
                                              max={cart?.product.marketable_number}
                                               step="1"
                                                value={cart.id == counterChangeId ? counter : cart.number} readOnly="readonly" />
                                            <button 
                                            disabled={counter == cart?.product.marketable_number ? true : false}
                                             onClick={() => { setCounter(counter+ 1)
                                                setCounterChangeId(cart.id)
                                            }}
                                              className="p-1 m-1 rounded-lg bg-pallete text-lg text-white"
                                               type="button">+</button>
                                        </section>
                            ...
                            <section className="flex  col-span-1   justify-end  items-end   h-full ">
                                <section className="p-2 mt-1 flex flex-col w-full h-full justify-end items-center lg:items-end xl:items-end md:items-center  ">
                                    <section className="text-red-600  mb-1">تخفیف   {priceFormatter(cart.cartItemFinalDiscount)} </section>
                                    <section className=" font-bold">  {priceFormatter(cart.cartItemFinalPrice)}  تومان</section>
                                </section>
...
export default CartItemFromDatabase;